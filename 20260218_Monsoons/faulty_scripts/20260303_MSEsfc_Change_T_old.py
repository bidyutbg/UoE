...
if pkl_path.exists():
    with open(pkl_path, 'rb') as f:
        all_bar_data_Relative_T = pickle.load(f)
else:
    all_bar_data_Relative_T = {}

# Compute once
bar_data, name = compute_regional_bar(all_results, EXP1, EXP2, plot=False)
all_bar_data_Relative_T[name] = bar_data

# Save
with open(pkl_path, 'wb') as f:
    pickle.dump(all_bar_data_Relative_T, f)
print(f"All keys now: {list(all_bar_data_Relative_T.keys())}")
