...
for model_name, model_meta in MODELS.items():
    ...
    for exp, meta in EXPERIMENTS.items():
        ...
        base_hus = make_base(get_var_name("hus", model_name))  # handles huss for CNRM
        ...
        if model_name == "CESM2-WACCM":
            if meta["scenario"] == "G6sulfur":
                ds_hus = mf.open_files_CESM_G6sulfur(base_hus)
            elif meta["scenario"] == "ssp585":
                ds_hus = mf.open_files_CESM_ssp585(base_hus)
            else:
                ds_hus = mf.open_files(str(base_hus))
        elif model_name == "IPSL-CM6A-LR":
            if meta["scenario"] == "ssp585":
                ds_hus = mf.open_files_IPSL_ssp585(base_hus)
            else:
                ds_hus = mf.open_files(str(base_hus))
        else:
            ds_hus = mf.open_files(str(base_hus))
        ...
        da_hus = mf.read_var(ds_hus, get_var_name("hus", model_name))  # handles huss  
        da_hus = da_hus.drop_vars(["plev", "height"], errors="ignore")
        ...
        # print(f"  [{model_name}] {exp}: hus loaded {da_hus.sizes['time']} months "
        #       f"({da_hus.time.values[0]} → {da_hus.time.values[-1]})")

        # # Compute MSEsfc = Cp*Tsfc + Lv*qsfc
        # var[exp] = Cp * da_tas + Lv * da_hus
        # var[exp].name = "MSEsfc"
        ...
        # Compute MSEsfc = Cp*Tsfc + Lv*qsfc
        var[exp] = Lv * da_hus ;#Cp * da_tas ;#+ 
        var[exp].name = "MSEsfc_Q"
...
pkl_path = SAVE_DIR / 'all_bar_data_Relative_Q.pkl'

# Load existing or start fresh
if pkl_path.exists():
    with open(pkl_path, 'rb') as f:
        all_bar_data_Relative_Q = pickle.load(f)
else:
    all_bar_data_Relative_Q = {}

# Compute once
bar_data, name = compute_regional_bar(all_results, EXP1, EXP2, plot=False)
all_bar_data_Relative_Q[name] = bar_data

# Save
with open(pkl_path, 'wb') as f:
    pickle.dump(all_bar_data_Relative_Q, f)
print(f"All keys now: {list(all_bar_data_Relative_Q.keys())}")
